...
def collect_smiles_from_directory(directory):
    smiles_list = []
    for filename in os.listdir(directory):
        if filename.endswith(".smi"):
            file_path = os.path.join(directory, filename)
            with open(file_path, 'r') as file:
                smiles_list.extend(file.readlines())
    return smiles_list


def save_smiles_to_file(smiles_list, output_file):
    with open(output_file, 'w') as file:
        file.writelines(smiles_list)
...
def main(args):
    ...
    def make_all():
        ...
        # Collect and save SMILES for train, valid, and test sets
        for subset in ['train', 'valid', 'test']:
            smiles = collect_smiles_from_directory(os.path.join(args.data, subset))
            save_smiles_to_file(smiles, f"{subset}.smi")
            make_binary_dataset(f"{subset}.smi", subset, dic, num_workers=args.workers)
            os.remove(f"{subset}.smi")
